...
class SupervisedExperiment(Experiment):
    ...
    def run(self):
        ...
        if self.finetune:
            fine_tune_model(detector, tokenizer, self.data, self.batch_size,
                            self.DEVICE, self.pos_bit, self.num_labels, epochs=self.epochs)
        ...
        if self.num_labels == 2:
            train_preds = get_supervised_model_prediction(
                detector, tokenizer, train_text, self.batch_size, self.DEVICE, self.pos_bit)
        ...
        print(f"{self.model} acc_train: {acc_train}, precision_train: {precision_train}, recall_train: {recall_train}, f1_train: {f1_train}, auc_train: {auc_train}")
        print(f"{self.model} acc_test: {acc_test}, precision_test: {precision_test}, recall_test: {recall_test}, f1_test: {f1_test}, auc_test: {auc_test}")
